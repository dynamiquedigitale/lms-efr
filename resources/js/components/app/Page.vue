				...
				<div class="row breadcrumbs-top">
					<div class="col-12">
						...
						<div class="breadcrumb-wrapper">
							<ol class="breadcrumb">
								<li class="breadcrumb-item"><Link href="/">{{ $t('Accueil') }}</Link></li>
								<li v-for="(item, i) in breadcrumb" :key="i" class="breadcrumb-item" :class="{ active: item.isLast }">
									<Link v-if="!item.isLast" :href="route(item.route)">{{ item.title }}</Link>
...
const breadcrumb = computed(() => props.breadcrumb.map(e => ({
	isLast: typeof e === 'string',
	route: typeof e === 'string' ? null : e.title,
